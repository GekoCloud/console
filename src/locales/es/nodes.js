...
export default {
  ...
  'Delete All Taints': 'Eliminar todas las manchas',
  ...
  TAINTS_MSG:
    'Si se agrega una mancha con &quot;clave = valor&quot; al nodo, significa que no se programará ningún pod en este nodo (política PodToleratesNodeTaints) o al menos se evita la programación tanto como sea posible (política TaintTolerationPriority), a menos que el pod tenga una coincidencia tolerancia con &quot;clave = valor&quot;.',
  TAINTS_TIPS:
    'Si hay al menos una contaminación no ignorada con efecto NoSchedule, entonces el sistema no programará pods en ese nodo. <br /> Si no hay una contaminación no ignorada con efecto NoSchedule pero hay al menos una contaminación no ignorada con efecto PreferNoSchedule, entonces el sistema intentará no programar pods en el nodo. <br /> si hay al menos una contaminación no ignorada con efecto NoExecute, los pods serán expulsados del nodo (si ya se está ejecutando en el nodo) y no se programarán en el nodo (si aún no se está ejecutando en el nodo).',
  ...
  NODE_MEMORYPRESSURE_DESC:
    'Si la presión de uso de memoria en el nodo es demasiado alta, la programación falla',
  NODE_DISKPRESSURE: 'DiskPressure',
  NODE_DISKPRESSURE_DESC:
    'Si existe presión sobre el tamaño del disco, es decir, si la capacidad del disco es baja',
  NODE_PIDPRESSURE: 'Presión PID',
  NODE_PIDPRESSURE_DESC:
    'Si existe presión sobre los procesos, es decir, si hay demasiados procesos en el nodo',
  NODE_READY: 'Listo',
  ...
  NODE_MEMORYPRESSURE_TIP:
    'Si existe presión en la memoria del nodo, es decir, si la memoria del nodo es baja.',
  ...
  WHAT_IS_NODE_TAINTS_A:
    'Las manchas permiten que un nodo repele un conjunto de pods. Las contaminaciones y las tolerancias funcionan juntas para garantizar que los pods no se programen en nodos inapropiados.',
  NODE_TYPE_DESC:
